...
#define WITHIMPORTFIELDS
    ...
    ! exportable field: ice concentration
  ...
  subroutine Realize(model, rc)
    ...
    ! read the namelist
    open(newunit=unit_num, file='3comp_time_example.nml', status='old', iostat=rc)
    read(unit_num, nml=domain, iostat=rc)
    read(unit_num, nml=ice, iostat=rc)
    close(unit_num)
    ...
    gridOut = ESMF_GridCreateNoPeriDimUfrm(maxIndex=maxIndexIce, &
      minCornerCoord=minCornerCoord, &
      maxCornerCoord=maxCornerCoord, &
      coordSys=ESMF_COORDSYS_CART, staggerLocList=(/ESMF_STAGGERLOC_CENTER/), &
      rc=rc)
    ...
    field = ESMF_FieldCreate(name="conc", grid=gridOut, &
      typekind=ESMF_TYPEKIND_R8, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
...
#ifdef NUOPC_TRACE
    call ESMF_TraceRegionEnter("ICE:Advance")
#endif
...
#ifdef NUOPC_TRACE
    call ESMF_TraceRegionExit("ICE:Advance")
#endif
  end subroutine
